let f_name="dee"; //type inference ..infers the type based in the value
...
let lName:string|null|undefined;

let personAge:number;
personAge=45;

let hasNotPurchased:boolean=true;

let flowerName=['tulips','cherry blossom','sunflower']
let quantity=[1,2,3,5];
quantity.push(100);

let farray=[]; //this is an any array ~ let farray:any[]=[];
...
let userChoice=iceCream.choco;
if(userChoice===iceCream.choco)
{console.log('Choco flavour selected')}

const enum newIceCream{
    choco, 
    vanilla, 
    fudge
}
let newice=newIceCream.fudge;// in JS this is will store the exact value and not convert it to function
